...
| **Полка** | Перетащи файлы в челку — они лежат там, пока не понадобятся; тянешь карточку наружу и файл уходит куда нужно. Наведи на карточку и нажми Пробел — откроется системный предпросмотр Quick Look; повторный Пробел его закроет. Клик выбирает карточку, ⌘-клик — несколько, и тогда перетаскивается вся группа. Снимок экрана, сделанный в буфер обмена, сохраняется файлом и попадает сюда же — в том числе снятый на айфоне, если скопировать его там |
...
### Анимация музыки по жанру

Для полос в острове можно выбрать ручный стиль либо «Автоматически по жанру».
В автоматическом режиме Cyclop определяет жанр только у трека, который публикует
**нативное приложение Яндекс Музыка**, и выбирает подходящий стиль; если жанр
не найден или сеть недоступна, остаётся «Универсальный» стиль. Текущий жанр и
выбранный стиль видны только в развернутой вкладке «Музыка».

Проверка жанра обращается к публичному каталогу Яндекс Музыки и передаёт только
метаданные текущего трека для поиска. Аккаунт, cookies и токены Cyclop не
отправляет и не использует. Для Apple Music, Spotify и браузеров lookup вообще
не вызывается: они продолжают использовать выбранный вручную стиль. Это также
относится к Яндекс Музыке, открытой во вкладке браузера.
